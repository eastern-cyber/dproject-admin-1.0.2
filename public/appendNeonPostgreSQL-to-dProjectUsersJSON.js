// appendNeonPostgreSQL-to-dProjectUsersJSON.js
...
// File paths - using the new JSON file with object structure
const TARGET_JSON_PATH = './dProjectUsers.json';
...
// Function to convert database record to match dProjectUsers.json format (with planA as object)
function convertDBRecordToDProjectUsersFormat(dbRecord) {
  const { user_id, referrer_id, email, name, token_id, plan_a } = dbRecord;
  
  // For dProjectUsers.json, we want to preserve the full planA object structure
  // but only include selected fields like in your sample
  let planAObject = null;
  
  if (plan_a && plan_a.dateTime) {
    planAObject = {
      dateTime: plan_a.dateTime,
      POL: parseFloat(plan_a.POL) || 0,
      rateTHBPOL: parseFloat(plan_a.rateTHBPOL) || 0
    };
    
    // Only include the object if it has valid data
    if (!planAObject.POL && !planAObject.rateTHBPOL) {
      planAObject = null;
    }
  }
...
async function main() {
  ...
  try {
    console.log('🚀 Starting PostgreSQL to dProjectUsers JSON append process...\n');
    ...
    newRecords.forEach(record => {
      const planAInfo = record.planA ? `Object with date: ${record.planA.dateTime}` : 'No planA data';
      console.log(`   - Token ID: ${record.tokenId}, User: ${record.name || 'N/A'}, PlanA: ${planAInfo}`);
    });
